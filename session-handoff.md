...
- Pending: ① **提交与发布 `v0.3.3`**：**已完成** —— 提交 `3897cd1`（先跑了提交门 ponytail → code-review → neat-freak）、`git push origin main`、GitHub Release `v0.3.3`（tag `3897cd1`，资产 `232,947,408` bytes，服务端 SHA-256 与本地一致）；本机 `git fetch --tags origin` 已拿到 tag `v0.3.3` → `3897cd1`（当时代理 `127.0.0.1:7897` 转发不稳，fetch/push 用直连 `git -c http.proxy= …` 才通，见下方环境备注）；② **跑门禁必须用 Node 24.14.1 或 24.15.0** —— 本机默认的 **v24.16.0 在解压 electron zip 时会静默卡死**，`electron-forge make` 因此空跑并仍返回 exit 0（`nvm use 24.14.1` 后一次通过）；③ **真机小点**（用户提到「稍后把真机测试的一些小点完善了再说」，等清单）；④ **云端 Provider 端到端实测**（`feat-027` 探针已绿，仍需用户用真实文章在设置页走一遍）；⑤ 任何代码改动都需新版本号（≥ `0.3.4`）并重跑门禁（门禁硬校验目标版本）；⑥ **UI 评审已完成（`0.3.1`），用户看过对照板后决定全部不整改** → 记录见 `docs/UI-REVIEW-2026-09-20.md`（对照板是临时文件，已删），不要重提这批发现（除用户指认具体条目）；⑦ 签名/notarization 用户 2026-09-20 决定不做（QA-008 accepted）
