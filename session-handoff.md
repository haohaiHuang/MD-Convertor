...
- Pending: ① 跑 `npm run desktop:release`（`0.3.1`）→ 记录新 ZIP 哈希并把 `[Unreleased]` 归档为 `[0.3.1]`；② 是否发布到 GitHub Releases；③ **下一轮 = UI 优化（用户已定，单开新会话）**。提交已完成（提交门已跑并报告）
