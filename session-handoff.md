...
- Current version: `0.3.3`（`package.json`、锁文件、`feature_list.json` 与发布门禁均为 `0.3.3`）。**已跑 `npm run desktop:release`（2026-09-20，exit 0）**：`out/make/zip/darwin/arm64/MD-Convertor-darwin-arm64-0.3.3.zip`，`232,947,408` bytes，SHA-256 `1bf807dfe294860c24345efe5caf2b0fcbd54f88476df7085c9bd03b47b37a72`。已安装到 `/Applications/MD-Convertor.app`（替换 `0.3.2`，未压缩 539 MB）。**已提交（`3897cd1`）并作为 `v0.3.3` 发布**（GitHub Release `v0.3.3`，tag `3897cd1`，资产 `232,947,408` bytes）。未签名，仅适合个人测试
- Active feature: none（`feat-024` – `feat-037` 均已 done）
- Pending: ① **`feat-035` 的版本决策已定（用户 2026-09-21 选 B）**：保持 `0.3.3`，不 bump、不重跑 `npm run desktop:release`，只把修复提交在 `v0.3.3` 之上（因此本机 `main` 领先已发布 tag 一个提交，这是刻意接受的状态；`next.config.ts` 属构建输入，源码与已发布的 `0.3.3` ZIP 不再逐字节对应，而打包产物文件级等价）；② **提交与发布 `v0.3.3`**：**已完成** —— 提交 `3897cd1`（先跑了提交门 ponytail → code-review → neat-freak）、`git push origin main`、GitHub Release `v0.3.3`（tag `3897cd1`，资产 `232,947,408` bytes，服务端 SHA-256 与本地一致）；本机 `git fetch --tags origin` 已拿到 tag `v0.3.3` → `3897cd1`（当时代理 `127.0.0.1:7897` 转发不稳，fetch/push 用直连 `git -c http.proxy= …` 才通，见下方环境备注）；③ **跑门禁必须用 Node 24.14.1 或 24.15.0** —— 本机默认的 **v24.16.0 在解压 electron zip 时会静默卡死**，`electron-forge make` 因此空跑并仍返回 exit 0（`nvm use 24.14.1` 后一次通过；本机当前 `node -v` = 24.15.0，可跑）；④ **真机小点**（用户提到「稍后把真机测试的一些小点完善了再说」，等清单）；⑤ **云端 Provider 端到端实测**（`feat-027` 探针已绿，仍需用户用真实文章在设置页走一遍）；⑥ 任何代码改动都需新版本号（≥ `0.3.4`）并重跑门禁（门禁硬校验目标版本）；⑦ **UI 评审已完成（`0.3.1`），用户看过对照板后决定全部不整改** → 记录见 `docs/UI-REVIEW-2026-09-20.md`（对照板是临时文件，已删），不要重提这批发现（除用户指认具体条目）；⑧ 签名/notarization 用户 2026-09-20 决定不做（QA-008 accepted）
- Branch: `main`；`feat-031` + `feat-032` 已提交（`af7f6db`）并推送，`v0.3.1` tag 指向 `af7f6db`（**不要移动/覆盖已发布的 tag 与产物**）；`feat-033` 已提交（`1c3ed80`）并发布为 `v0.3.2`（tag 指向 `1c3ed80`）；`feat-034`（`0.3.3`）已提交（`3897cd1`）并作为 `v0.3.3` 发布
...
`feat-034`（去掉重复 Electron 运行时 + 版本 `0.3.3`）已实现、**已跑完发布门禁（exit 0，ZIP `232,947,408` bytes / SHA-256 `1bf807df…7a72`）**，已安装到 `/Applications/MD-Convertor.app` 替换 `0.3.2`，文档（`CHANGELOG`(+zh)、`README`(+zh)、`docs/ARCHITECTURE`(+zh)、`docs/TESTING`(+zh)、`PROGRESS`、`session-handoff`、`feature_list.json`、`AGENTS.md`）已同步。下一步：① **提交与发布已完成** —— `0.3.3` 提交在 `3897cd1`，已推送 `origin/main`，GitHub Release `v0.3.3`（tag `3897cd1`，资产 `232,947,408` bytes，服务端 SHA-256 与本地一致）已上线；本机 tag 已同步（`v0.3.3` → `3897cd1`）；② **真机小点**：等用户给清单后再评估是否单开一轮；③ 若再改代码，先 bump 版本号（≥ `0.3.4`）再跑 `npm run desktop:release`（门禁硬校验目标版本），**不要移动或覆盖 `v0.3.1` / `v0.3.2` 的 tag 与产物**；④ **签名/notarization 用户 2026-09-20 决定不做**（QA-008 accepted / not planned）；若将来要分发再补 Developer ID Application 证书与 notarytool 凭据，并重跑门禁更新哈希。⑥ **`feat-036`（抓取失败时提示改用粘贴）与 `feat-037`（云端卡片「清除」改为整卡重置）已实现、已跑完 `./init.sh` 与三浏览器 e2e，但尚未提交、未跑发布门禁** —— 若决定发布，先把版本号升到 `0.3.4` 或更高（门禁硬校验目标版本），再跑 `npm run desktop:release`。⑤ 主页有像素级断言 `e2e/home.spec.ts`：转换按钮右边缘与粘贴框右边缘差值 < 4px 且与「来源 URL」输入框同行，改 `.sourceInput`/`.sourceRow` 前先看那条用例。⑥ **`feat-036`（抓取失败时提示改用粘贴）与 `feat-037`（云端卡片「清除」改为整卡重置）已实现、已跑完 `./init.sh` 与三浏览器 e2e，并已提交推送（见下方本轮段落）；尚未跑发布门禁** —— 若决定发布，先把版本号升到 `0.3.4` 或更高（门禁硬校验目标版本），再跑 `npm run desktop:release`。 不要重做 S1–S6 与 `feat-024` – `feat-037`；不要放宽端点、密钥或归档守卫；不要把已退役的历史 ZIP 条目从 `PROTECTED_HISTORICAL_ZIP_MANIFEST` 中删除。
