...
- **v0.3.3 是当前版本（源码；`0.3.1`、`0.3.2` 与 `0.3.3` 均已发布为 GitHub Release，`0.3.0` 的 ZIP 是历史构建），0.1.3 为不可变历史基线**：`main` 代表当前版本；`main` 还包含 `v0.3.3` 之后的 `feat-035`（`next.config.ts` 的追踪修复），因此源码与已发布的 `0.3.3` ZIP 不再逐字节对应，而打包产物保持文件级等价；`v0.1.3` 标签固定在提交 `ce041c9`。0.2.1 的 ZIP 归档于 `~/Downloads/MD-Convertor-archive/releases/`；0.1.0–0.2.0 的 ZIP 与原 0.1.3 只读副本已从本机丢失且无法恢复，发布门禁对这些缺失项只报退役。后续版本不得移动历史标签、覆盖或删除仍然存在的受保护产物；既有回归继续包含在扩展测试集中。
