...
注：`build:extension` 与 `test:extension` 由 S1（`docs/features/browser-extension/S1-convert-core.md` 的 T1.0）引入，**在此之前不存在**；`extension/` 目录、`manifest.json`、`playwright.extension.config.ts` 同理，目前只是规划。
