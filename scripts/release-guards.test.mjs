...
<key>CFBundleVersion</key><string>${version}</string>
  ...
  async function makeFreshArtifactFixture({ zipVersion = "0.3.6", zipExecutable = "/bin/echo" } = {}) {
    ...
    const zipPath = path.join(root, "out", "make", "zip", "darwin", "arm64", "MD-Convertor-darwin-arm64-0.3.6.zip");

    await mkdir(path.join(sideContents, "MacOS"), { recursive: true });
    await writeFile(path.join(sideContents, "Info.plist"), plist("0.3.6"));
  ...
  it("rejects a ZIP whose bundled version differs from the verified side application", async () => {
    ...
    try {
      expect(() => verifyFreshArtifact({ root, version: "0.3.6", startedAtMs: 0 }))
...
describe("desktop release protection orchestration", () => {
  ...
  it("accepts only the current 0.3.6 release target", async () => {
    const { options } = makeReleaseFixture();
    ...
    expect(RELEASE_VERSION_ERROR).toContain("0.3.6");
  });
});
