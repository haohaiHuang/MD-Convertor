...
- **为什么要做**（收窄前的实证）：0.3.6 的 `app.asar` 有 **253 个条目**、装着**整个仓库根** —— 含 `.workbuddy/memory/*.md`、`session-handoff.md`、`PROGRESS.md`、`docs/`、`src/`、`e2e/`、`tests/`、`scripts/` 与全部明文配置。**但「这些是私有文档」的定性是错的，2026-09-22 已更正**：仓库 `haohaiHuang/MD-Convertor` 是 **public**，`PROGRESS.md` / `session-handoff.md` / `feature_list.json` / `AGENTS.md` / `docs/**` **全部已在 `origin/main` 上公开可浏览**（`git cat-file -e origin/main:<file>` 逐个确认），所以把它们打进 ZIP **不构成任何新增暴露** —— 已发布的 `v0.3.1`–`v0.3.5` 的 asar 同样装着整个仓库根，但那批文件本来就在公开 git 里。**真正的、且是唯一的新增暴露是 `.workbuddy/memory/*.md`**：它在 `.gitignore` 第 10 行，**从未进入任何提交**（`git log --all -- .workbuddy` 为空），里面是我方跨会话工作日志（含用户 iCloud 目录路径、机器状态排查记录等），只能靠打包 ignore 拦住。附带两条理由：残留的 6 处 `value.includes("~")` 来自文档而非代码，让「这包带没带修复」的判定含混；仓库根的 `public/` 是 `prepare-desktop.mjs` 已复制到 `.desktop/server/public` 的死副本。
