...
- v0.3.2 是当前版本（`0.3.0` 与 `0.3.1` 的记录产物属历史构建），但产物仍只面向 Apple Silicon Mac 个人测试；历史标签和外部 0.1.0–0.2.0 ZIP 归档继续作为不可变回归基准。
