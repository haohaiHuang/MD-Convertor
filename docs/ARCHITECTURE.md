...
- v0.3.3 is the current version (the recorded `0.3.0`, `0.3.1` and `0.3.2` artifacts are historical). Historical tags and the external 0.1.0–0.2.0 ZIP archive remain immutable regression references.
...
- The same tracing follows Playwright's static imports but not the `playwright-core/browsers.json` data file it reads while loading, so `next.config.ts` lists that one file in `outputFileTracingIncludes` for `/api/convert`. Without it a standalone server cannot import Playwright at all and answers 500 for every link; the full-package copy in desktop preparation only masked that for packaged builds.
