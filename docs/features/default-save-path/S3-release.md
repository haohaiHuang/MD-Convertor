...
| T3.6 | 提交与发布 ✅ **已完成**（2026-09-22） | 提交（含版本与文档）→ `git push` → `gh release create v0.3.6 <zip>`（notes 写明未签名） | 实得：收尾提交 `3578822`（17 个文件、+283 / −220）推到 `origin/main`；`gh release create v0.3.6 … --target main` 建在 `3578822` 上，资产 `MD-Convertor-darwin-arm64-0.3.6.zip` **235,956,668** bytes、state `uploaded`、**server 端 digest `sha256:9b89d55c…f351` 与本机 ZIP 逐字节一致**，标记 **Latest**（`releases/latest` 返回 `v0.3.6`），非 draft 非 prerelease。坑：`--target <短 SHA>` 会被 API 拒（`target_commitish is invalid`），用分支名 `main` 即可，且首次失败不会留下半个 tag |
| T3.7 | 文档收口 ✅ **已完成**（2026-09-22） | `CHANGELOG.md`(+zh) `[Unreleased]` → `[0.3.6]`；`docs/TESTING.md`(+zh) 门禁计数与产物段；`README`(+zh) 如有行为差异；`docs/ARCHITECTURE.md`(+zh) 增补 IPC 通道与安全边界（filename 校验、日志纪律）；`docs/QUALITY-AUDIT.md` 归档本轮；`AGENTS.md` 版本行 → `0.3.6`；`PROGRESS.md` / `session-handoff.md` 重写；`feature_list.json` `feat-041` → done | 全部完成；`feature_list.json` 现为 `in-progress=0`、`done=24`、`planned=1`（`feat-040` 浏览器扩展方向）、`activeFeature=null`、`currentVersion=0.3.6`，下一会话可按 Startup Workflow 无歧义恢复 |
