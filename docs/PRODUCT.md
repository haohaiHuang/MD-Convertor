...
- A separate Chromium (Chrome, Edge, Brave) browser extension for the browser-first workflow, accepted on 2026-09-24 and loaded unpacked from `extension/dist`: one toolbar click on the page you are reading saves it as `<title>.md` plus a `<title>.images/` folder. It runs inside the page you already have open, so it needs neither the desktop app nor a server, and images that require your signed-in session still download. Failed images keep their original URL and carry a `<!-- 图片未下载：… -->` note; re-exporting the same page overwrites the previous pair. It is not published to a store and has no popup or options page.
...
URLs, clipboard HTML/plain text, webpages, images, and conversion results are not uploaded to an MD-Convertor service. The application code creates no conversion history, database, or analytics record. Pasted content exists only for the current local request, and diagnostic logs do not record body text, HTML, or source URLs. The app does not read browser cookies or authenticated state. The browser extension reads the current tab's content only when you click its toolbar icon; its image downloads ride on the browser's own session, and it never reads, exports, or stores cookies or login state. Electron and the embedded browser create local runtime caches and browser profiles, but the app does not use them to synchronize or restore conversion content. Closing the app clears the current result except for `.md` files explicitly saved by the user.
